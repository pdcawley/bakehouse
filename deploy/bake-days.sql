...
CREATE OR REPLACE FUNCTION first_bake_date_after( a_date DATE ) RETURNS DATE AS $$
    WITH next_dow(dow) AS (SELECT coalesce( ( SELECT dow FROM bakehouse.bake_day WHERE dow > EXTRACT(dow FROM $1)
                                              ORDER BY dow LIMIT 1)
                                          , (SELECT MIN(dow) + 7 FROM bakehouse.bake_day)) AS dow)
...
CREATE OR REPLACE FUNCTION first_bake_date_after(a_time TIMESTAMP) RETURNS DATE AS $$
    SELECT bakehouse.first_bake_date_after($1::date)
$$ LANGUAGE SQL;


CREATE OR REPLACE FUNCTION next_bake_date() RETURNS DATE AS $$
   SELECT bakehouse.first_bake_date_after(current_date)
$$
LANGUAGE SQL;
